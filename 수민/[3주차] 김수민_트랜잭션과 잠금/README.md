...
하나의 트랜잭션에서 두 번의 동일한 조회를 하였을 때, 서로 다른 결과가 조회되는 현상
    ...
    ✔️ Dirty Read와 Non-repeatable Read의 차이점
        
        Dirty Read는 commit되지 않은 데이터를 읽어 존재하지 않는 값(보면 안되는 값)을 읽어버리는 것이며, Non-repeatable Read는 변경된 정상데이터를 읽어버려 데이터가 트랜잭션 중간에 변경되는 것
...
각 트랜잭션에 ID를 부여하여 ID보다 작은 트랜잭션에서만 데이터를 조회한다. 즉, 트랜잭션이 시작되기 전에 커밋된 데이터만 조회한다.
    ...
    ✔️ Read Committed와 Repeatable Read의 차이점
...
✔️ 하나의 connection 안에서 transaction을 짧게 2개로 나누는 것이 길게 1개로 진행하는 것보다 좋은 이유
